...
export function renderHeader(container, user) {
            ...
            isLoggedIn
              ? `<div class="flex items-center gap-4">
                  <span class="text-sm font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-3 py-1">
                    Credits: <span data-user-credits>${credits}kr</span>
                  </span>
  ...
  const createListingBtns = container.querySelectorAll(
    "[data-nav='create-listing']",
  );
  ...
  registerBtns.forEach((btn) =>
    btn.addEventListener("click", () => navigateTo("register")),
  );
  profileBtns.forEach((btn) =>
    btn.addEventListener("click", () => navigateTo("profile")),
  );
  createListingBtns.forEach((btn) =>
    btn.addEventListener("click", () => navigateTo("listing/new")),
  );
